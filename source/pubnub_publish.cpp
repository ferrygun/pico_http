...
#define JSON_PAYLOAD "{ \"message\": \"Hello\" }"
...
static err_t tls_client_connected(void *arg, struct altcp_pcb *pcb, err_t err) {
    ...
    char payload[512];
    snprintf(payload, sizeof(payload), TLS_CLIENT_HTTP_REQUEST, strlen(JSON_PAYLOAD), JSON_PAYLOAD);

    err = altcp_write(state->pcb, payload, strlen(payload), TCP_WRITE_FLAG_COPY);
    if (err != ERR_OK) {
...
static err_t tls_client_recv(void *arg, struct altcp_pcb *pcb, struct pbuf *p, err_t err) {
    ...
    if (p->tot_len > 0) {
        /* For simplicity, this example creates a buffer on the stack the size of the data pending here, 
           and copies all the data to it in one go.
           Do be aware that the amount of data can potentially be large (TLS record size can be 16 KB),
...
static bool tls_client_open(const char *hostname, void *arg) {
    ...
    // You can omit them if you are in a callback from lwIP. Note that when using pico_cyw43_arch_poll
